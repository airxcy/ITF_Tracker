...
class HybridDataLayerTest : public MultiDeviceTest<TypeParam> {
 ...
 protected:
  ...
  virtual void SetUp() {
    ...
    *filename_ += "/db";
    ...
    blob_top_vec_.push_back(blob_top_num_);
  }
  ...
  void FillLevelDB(const bool unique_pixels) {
    ...
    for (int i = 0; i < 5; ++i) {
      BlobZc sample;
      sample.set_total_num(30+i);
      BlobProto* dataMap = sample.mutable_img_blob();
      ...
      BlobProto* labelMap = sample.mutable_label_blob();
  ...
  void TestRead() {
    ...
    HybridDataLayer<Dtype> layer(param);
    ...
    EXPECT_EQ(blob_top_num_->channels(),1);
    EXPECT_EQ(blob_top_num_->height(),1);
    EXPECT_EQ(blob_top_num_->width(),1);
    ...
    for (int iter = 0; iter < 100; ++iter) {
      ...
      for (int i = 0; i < 5; ++i){
          EXPECT_EQ(30+i, blob_top_num_->cpu_data()[i])<<"debug:iter" << iter << "i" << i;
      }
    }
  }

  virtual ~HybridDataLayerTest() { delete blob_top_data_; delete blob_top_label_; }

  DataParameter_DB backend_;
  shared_ptr<string> filename_;
  Blob<Dtype>* const blob_top_data_;
  Blob<Dtype>* const blob_top_label_;
  Blob<Dtype>* const blob_top_num_;
  vector<Blob<Dtype>*> blob_bottom_vec_;
  vector<Blob<Dtype>*> blob_top_vec_;
  int seed_;
};

TYPED_TEST_CASE(HybridDataLayerTest, TestDtypesAndDevices);

TYPED_TEST(HybridDataLayerTest, TestReadLevelDB) {
